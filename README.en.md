...
| Implemented     | **1** (`json-formatter`) — the foundation and all deployment paths are done and verified |
...
`install.sh`, `latest.txt`, `toolbox-*.tar.gz` and its `.sha256`):
...
> ⚠️ **This repository is currently private**, so anonymous requests to
...
| Check for updates               | `toolboxctl check-update --source <source>`     |
| **Upgrade**                     | `toolboxctl upgrade --source <source>`          |
