/**
 * json-to-toml E2E（DEVELOPMENT.md §8.2 的 8 文件基线之一）
 *
 * 前置：先构建并启动预览服务
...
test.describe('json-to-toml', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${BASE}/tools/json-to-toml`)
  })

  test('页面标题为该工具名', async ({ page }) => {
    await expect(page).toHaveTitle(/JSON 转 TOML/)
  })

  test('示例 → 生成 TOML', async ({ page }) => {
    await page.getByTestId('example').click()
    await expect(page.getByTestId('output')).toContainText('[meta]')
    await expect(page.getByTestId('output')).toContainText('[[servers]]')
  })
