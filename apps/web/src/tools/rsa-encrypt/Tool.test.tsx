...
/**
 * rsa-encrypt 组件测试（DEVELOPMENT.md §8.2 的 8 文件基线之一）
 */
...
/** 示例签名（PKCS#1 v1.5 确定性）的前缀 */
const SIGNATURE_PREFIX = 'HdUurE0L7y2qWPwayii8LbVzy0JNJype'

describe('rsa-encrypt · Tool', () => {
  ...
  it('示例 → 运行 → 输出确定性签名', async () => {
    render(<Tool />)
    fireEvent.click(byTestId('example'))
    fireEvent.click(byTestId('run'))
    await waitFor(() => expect(output()).toContain(SIGNATURE_PREFIX), { timeout: 10000 })
  })

  it('切到验签方向并把签名填回去 → 验签通过', async () => {
    render(<Tool />)
    fireEvent.click(byTestId('example'))
    fireEvent.click(byTestId('run'))
    await waitFor(() => expect(output()).toContain(SIGNATURE_PREFIX), { timeout: 10000 })
  ...
  it('签名方向缺少私钥时进入错误态（role=alert）', async () => {
    ...
    fireEvent.change(byTestId('input-privateKey'), { target: { value: '' } })
    fireEvent.click(byTestId('run'))
