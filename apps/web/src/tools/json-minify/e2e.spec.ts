/**
 * json-minify E2E（DEVELOPMENT.md §8.2 的 8 文件基线之一）
 *
 * 前置：先构建并启动预览服务
...
test.describe('json-minify', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${BASE}/tools/json-minify`)
  })
  ...
  test('从首页能通过搜索进入该工具', async ({ page }) => {
    ...
    await page.getByText('JSON 压缩').first().click()
    await expect(page).toHaveURL(/\/tools\/json-minify$/)
  })
})
