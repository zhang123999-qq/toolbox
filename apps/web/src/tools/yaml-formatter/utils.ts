...
/**
 * YAML 格式化 / 校验核心（纯函数，无 React、无 DOM 依赖）。
 *
...
function fail(line: number, column: number, message: string): never {
  throw new YamlFormatterError(`第 ${line} 行第 ${column} 列：${message}`)
}
...
function collect(value: YamlValue, depth: number, stats: Stats): void {
  if (depth > stats.depth) stats.depth = depth
  if (Array.isArray(value)) {
    stats.seqs += 1
    for (const item of value) collect(item, depth + 1, stats)
    return
  }
  if (value !== null && typeof value === 'object') {
    for (const key of Object.keys(value)) {
      stats.keys += 1
      collect((value as Record<string, YamlValue>)[key], depth + 1, stats)
    }
  }
}
...
function typeName(value: YamlValue): string {
  if (value === null) return 'null'
  if (Array.isArray(value)) return 'sequence（序列）'
  if (typeof value === 'object') return 'mapping（映射）'
  if (typeof value === 'string') return 'string（字符串）'
  if (typeof value === 'number') return 'number（数字）'
  return 'boolean（布尔）'
}

function report(src: string, value: YamlValue): string {
  const stats: Stats = { keys: 0, seqs: 0, depth: 0 }
  collect(value, 1, stats)
  const lines = [
    '✓ YAML 合法，未发现语法错误',
    `· 顶层类型：${typeName(value)}`,
    `· 行数：${src.replace(/\r\n?/g, '\n').split('\n').length}`,
 ...
 */
export function transform(input: YamlFormatterInput, options: YamlFormatterOptions): string {
  if (!input.text.trim()) return ''
  if (input.text.length > MAX_INPUT) {
    throw new YamlFormatterError('输入超过 1,000,000 字符上限')
  }
  const parsed = parseYaml(input.text)
  if (options.mode === 'validate') return report(input.text, parsed.value)
  return stringifyYaml(parsed.value, Number(options.indent), options.sortKeys, parsed)
}
