/**
 * xml-to-json E2E（DEVELOPMENT.md §8.2 的 8 文件基线之一）
 *
 * 前置：先构建并启动预览服务
...
test.describe('xml-to-json', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${BASE}/tools/xml-to-json`)
  })
  ...
  test('非法输入时输出区给出错误提示', async ({ page }) => {
    await page.getByTestId('input').fill('<a><b></a>')
  ...
  test('从首页能通过搜索进入该工具', async ({ page }) => {
    ...
    await page.getByText('XML 转 JSON').first().click()
    await expect(page).toHaveURL(/\/tools\/xml-to-json$/)
  })
})
