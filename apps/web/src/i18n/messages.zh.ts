...
export const zh = {
  ...
  'tool.textA': '原文',
  'tool.textB': '新版',
  'tool.otherText': '另一份文本',
  'tool.running': '调用中…',
  'tool.asyncIdle': '填好接口信息后点「运行」',
  'tool.password': '口令',
  'tool.watermark': '水印内容',
  'tool.noMatch': '没有匹配项，换个关键词试试',
  'tool.picked': '已选',
  ...
  // 下列为铺量期预置的通用选项文案，覆盖文本 / 编码 / 转换等多域工具
  'option.countSpaces': '计入空格',
  'option.speed': '速度',
  'option.topN': '前 N 个',
  'option.ignoreCase': '忽略大小写',
  'option.useStopWords': '过滤停用词',
  'option.mask': '打码命中词',
  ...
  'option.width': '宽度',
  'option.filler': '填充字符',
  'option.ellipsis': '省略符号',
  'option.algorithm': '算法',
  'option.uppercase': '大写输出',
  'option.steps': '流水线步骤',
  'option.rate': '语速',
  'option.level': '容错等级',
  'option.breakLong': '断开超长词',
  'option.trim': '比较前去掉首尾空白',
  'option.length': '长度',
  'option.limit': '数量上限',
  'option.metric': '统计指标',
  'option.category': '分类',
  'option.pattern': '模式串',
  'option.replacement': '替换为',
  'option.direction': '方向',
  'option.language': '语言',
  'option.encoding': '编码',
  'option.prefix': '前缀',
  'option.suffix': '后缀',
  'option.keepEmpty': '保留空行',
  'option.trimLines': '去除行首尾空白',
  'option.useRegex': '使用正则',
  'option.charset': '字符集',
  'option.format': '格式',
  'option.unit': '单位',
  'option.sortBy': '排序依据',
  ...
  'option.source': '源语言',
  // —— 编码 / 加密域（02）通用选项（多工具复用，勿按工具各建一套）——
  'option.key': '密钥',
  'option.secret': '密钥 / 口令',
  'option.iv': '初始向量（IV）',
  'option.salt': '盐',
  'option.iterations': '迭代次数',
  'option.password': '口令',
  'option.period': '周期（秒）',
  'option.digits': '位数',
  'option.words': '词数',
  ...
  'option.endian': '字节序',
  'option.url': '目标地址',
  'option.timeout': '超时（毫秒）',
  'option.memory': '内存开销（KiB）',
  'option.parallelism': '并行度',
  'option.curve': '曲线',
  'option.bits': '长度（bit）',
  'option.errorLevel': '容错级别',
  'option.noAmbiguous': '排除易混字符',
  'option.eachClass': '每类至少一个',
  'option.includeLower': '包含小写字母',
  'option.includeUpper': '包含大写字母',
  'option.includeNumbers': '包含数字',
  'option.includeSymbols': '包含符号',
  'option.withHeader': '带表头',
  'option.quote': '引号风格',
  'option.method': '算法模式',
  'option.age': '有效期',
  'option.publicKey': '公钥（PEM）',
  'option.privateKey': '私钥（PEM）',
  'option.signature': '签名',
  'option.issuer': '签发者',
  'option.account': '账户名',
  'option.counter': '计数器',
  'option.payload': '载荷（Payload）',
  'option.subject': '主题（Subject）',
  'option.days': '有效期（天）',
  'option.cost': '计算开销（cost）',
