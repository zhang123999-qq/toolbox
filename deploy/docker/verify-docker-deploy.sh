#!/usr/bin/env bash
# ============================================================================
# C 组：Docker 真实部署验证（目标机 root@192.168.100.4，端口 8081）
#
...
exec_case C-03 "docker run 启动容器（8081:80）" 0 -- \
  docker run -d --name "$NAME" -p "$HPORT":80 "$IMG"
# 等健康检查
...
assert_case C-06 "宿主机 8081 端口监听（端口映射生效）" -- sh -c "ss -lnt | grep -q ':$HPORT '"
...
mkdir -p "$LOGVOL"
rm -f "$LOGVOL"/*
docker run -d --name tbv-docker-vol -p 8082:80 -v "$LOGVOL":/var/log/nginx "$IMG" >/dev/null
sleep 3
curl -s -o /dev/null "http://127.0.0.1:8082/"
sleep 1
...
exec_case C-19 "复用已占用端口启动应失败并报错" any -- \
  docker run -d --name tbv-docker-dup -p "$HPORT":80 "$IMG"
...
assert_case C-21 "停止后宿主机 8081 端口释放" -- sh -c "! ss -lnt | grep -q ':$HPORT '"
