...
> 本仓库当前为**私有**：默认方式对匿名请求会返回 404（已实测）。三种解法：把仓库转为 public、用 `GITHUB_TOKEN`、或改用 `--source` / `--from`。详见 [`../RELEASE.md`](../RELEASE.md) §四。
...
| `--source`        | 自建发布源基址（内网分发用）                          | 例 `http://192.168.1.10:8099`                      |
...
| `无法确定最新版本`       | 私有仓库 / 网络不通。用 `--version` 指定版本，或改用 `--source` / `--from` |
