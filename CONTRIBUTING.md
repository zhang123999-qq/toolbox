...
### 6.5 AI 助手的自动提交与推送（强制约束）

本项目由 AI 助手参与开发，**改动不允许停留在工作区**。每完成一个原子性改动并通过基本校验，
必须立即提交并推送到远程分支——攒几次改动再一次性提交，出问题时定位不到是哪一步引入的，
回滚也只能整块回滚，等于放弃了版本控制的意义。

1. **触发时机**：一个原子性改动（一个工具 / 一处修复 / 一份文档）完成、且 §七 中与本次改动
   相关的门禁通过后立即提交。判断不了影响面就跑完整的 `pnpm verify`。
2. **执行动作**：
   ```bash
   git status --short                      # 先逐项确认待提交文件
   git add <本次改动的具体路径>             # 按路径暂存，不要用 git add -A
   git -c user.name=<名> -c user.email=<邮箱> commit -m "<type>(<scope>): <中文简要描述>"
   git push origin <当前分支>
   ```
   提交信息按 §六 的 Conventional Commits 写：类型 + 中文简要描述，正文说清为什么改。
   本机未配全局提交身份时，用上面 `-c user.name/-c user.email` 临时传入。
3. **提交范围**：只包含本次改动相关的文件。构建产物、依赖目录、日志、`.env*` 等含敏感信息的
   文件一律排除——它们已被 `.gitignore` 覆盖；若 `git status` 里仍出现，说明忽略规则有漏，
   **先补规则再提交**，不要靠手挑绕过。
4. **失败处理**：push 失败、远端有新提交或出现冲突时**立即中止并报告原因**，由人决定怎么合并。
   禁止 `--force` / `--force-with-lease` 强推，禁止跳过 hooks，禁止为绕过冲突丢弃他人改动。
   网络不通时可配代理后重试（`https_proxy=http://127.0.0.1:10808`），重试仍失败同样按中止处理。
5. **验证要求**：
   - 提交前：`git status --short` 与 `git diff --cached --stat` 确认改动完整、没有夹带无关文件；
   - 提交后：输出提交哈希（`git log --oneline -1`）与改动文件数，并用
     `git ls-remote origin <分支>` 与本地 HEAD 比对，确认远端确实已同步。
